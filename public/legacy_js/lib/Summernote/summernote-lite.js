  ...
  HelpDialog_createClass(HelpDialog, [{
    ...
    value: function initialize() {
      ...
      this.$dialog = this.ui.dialog({
        ...
        footer: body,
        callback: function callback($node) {
          $node.find('.dialog-body,.note-dialog-body').css({
            'max-height': 300,
            'overflow': 'scroll'
          });
        }
...
// CONCATENATED MODULE: ./src/js/lite/ui/dialogUI.js
function dialogUI_classCallCheck(instance, Constructor) { if (!(instance instanceof Constructor)) { throw new TypeError("Cannot call a class as a function"); } }

function dialogUI_defineProperties(target, props) { for (var i = 0; i < props.length; i++) { var descriptor = props[i]; descriptor.enumerable = descriptor.enumerable || false; descriptor.configurable = true; if ("value" in descriptor) descriptor.writable = true; Object.defineProperty(target, descriptor.key, descriptor); } }

function dialogUI_createClass(Constructor, protoProps, staticProps) { if (protoProps) dialogUI_defineProperties(Constructor.prototype, protoProps); if (staticProps) dialogUI_defineProperties(Constructor, staticProps); return Constructor; }



var dialogUI_dialogUI = /*#__PURE__*/function () {
  function dialogUI($node
  /*, options */
  ) {
    dialogUI_classCallCheck(this, dialogUI);

    this.$dialog = $node;
    this.$backdrop = external_root_jQuery_commonjs2_jquery_commonjs_jquery_amd_jquery_default()('<div class="note-dialog-backdrop"/>');
  }

  dialogUI_createClass(dialogUI, [{
    key: "show",
    value: function show() {
      var _this = this;

      this.$backdrop.appendTo(document.body).show();
      this.$dialog.addClass('open').show();
      this.$dialog.trigger('note.dialog.show');
      this.$dialog.off('click', '.close').on('click', '.close', this.hide.bind(this));
      this.$dialog.on('keydown', function (event) {
        if (event.which === 27) {
          event.preventDefault();
    ...
    key: "hide",
    value: function hide() {
      this.$dialog.removeClass('open').hide();
      this.$backdrop.hide();
      this.$dialog.trigger('note.dialog.hide');
      this.$dialog.off('keydown');
    }
  }]);
...
/* harmony default export */ var ui_dialogUI = (dialogUI_dialogUI);
...
var dialog = renderer["a" /* default */].create('<div class="note-dialog" aria-hidden="false" tabindex="-1" role="dialog"/>', function ($node, options) {
  if (options.fade) {
    $node.addClass('fade');
  }
  ...
  $node.html(['<div class="note-dialog-content">', options.title ? '<div class="note-dialog-header"><button type="button" class="close" aria-label="Close" aria-hidden="true"><i class="note-icon-close"></i></button><h4 class="note-dialog-title">' + options.title + '</h4></div>' : '', '<div class="note-dialog-body">' + options.body + '</div>', options.footer ? '<div class="note-dialog-footer">' + options.footer + '</div>' : '', '</div>'].join(''));
  $node.data('dialog', new ui_dialogUI($node, options));
});
...
var ui = function ui(editorOptions) {
  return {
    ...
    onDialogShown: function onDialogShown($dialog, handler) {
      $dialog.one('note.dialog.show', handler);
    },
    onDialogHidden: function onDialogHidden($dialog, handler) {
      $dialog.one('note.dialog.hide', handler);
    },
    showDialog: function showDialog($dialog) {
      $dialog.data('dialog').show();
    },
    hideDialog: function hideDialog($dialog) {
      $dialog.data('dialog').hide();
    },
    ...
    getDialogBody: function getDialogBody($dialog) {
      return $dialog.find('.note-dialog-body');
    },
