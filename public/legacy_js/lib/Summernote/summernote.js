...
var dialog = renderer["a" /* default */].create('<div class="dialog note-dialog" aria-hidden="false" tabindex="-1" role="dialog"/>', function ($node, options) {
  if (options.fade) {
    $node.addClass('fade');
  }
  ...
  $node.html(['<div class="dialog-dialog">', '<div class="dialog-content">', options.title ? '<div class="dialog-header">' + '<button type="button" class="close" data-dismiss="dialog" aria-label="Close" aria-hidden="true">&times;</button>' + '<h4 class="dialog-title">' + options.title + '</h4>' + '</div>' : '', '<div class="dialog-body">' + options.body + '</div>', options.footer ? '<div class="dialog-footer">' + options.footer + '</div>' : '', '</div>', '</div>'].join(''));
});
...
var ui_ui = function ui(editorOptions) {
  return {
    ...
    onDialogShown: function onDialogShown($dialog, handler) {
      $dialog.one('shown.bs.dialog', handler);
    },
    onDialogHidden: function onDialogHidden($dialog, handler) {
      $dialog.one('hidden.bs.dialog', handler);
    },
